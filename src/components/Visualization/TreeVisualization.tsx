...
import { TreeSnapshot, SerializedNode } from '../../algorithms/types';
    ...
    <div className="flex min-h-[300px] w-full items-center justify-center">
      ...
      >
        <g transform={`translate(${marginLeft}, ${marginTop})`}>
          ...
          {/* Draw nodes */}
          {nodes.map((node) => {
            const isHighlighted = highlightedNodes.includes(node.data.id);
              ...
              >
                ...
                {isNYT ? (
                  <rect
                    ...
                    style={isHighlighted ? { filter: 'drop-shadow(0 0 8px #2196f3)' } : undefined}
                  />
                ) : (
                  <circle
                    r={15}
                ...
                {/* Weight label */}
                <text dy="2.5em" textAnchor="middle" fill="#aaa" fontSize="10" pointerEvents="none">
                  w:{node.data.weight}
                </text>
              </g>
            );
...
export const TreeVisualization = memo(TreeVisualizationComponent, (prevProps, nextProps) => {
  ...
  return treeEqual && highlightedEqual;
});
